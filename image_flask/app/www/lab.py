...
def main():
    ...
    valid = verify_mac(key, my_name, uid, cmd, message, mac)
    if not valid:
...
def verify_mac(key, my_name, uid, cmd, message, mac):
    message_message = '' if not message else '&message=' + message
    message = ''
    if my_name:
        message = 'myname={}&'.format(my_name)
    message += 'uid={}&lstcmd='.format(uid) + cmd + message_message
    payload = key + ':' + message
...
def read_file(file):
    ...
    if not os.path.exists(path):
        return 'Money has been transferred [{}]'.format(file)
    result = []
    f = open(path, 'r')
    lines = f.readlines()
    for line in lines:
